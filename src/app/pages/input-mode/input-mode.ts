...
export class InputMode {
  ...
  learned(): void {
    ...
    this.progress.markLearned(lesson.id, 'input', item.id);
  ...
  restart(): void {
    ...
    this.progress.reset(lesson.id, 'input');
  ...
  private loadDeck(id: number): void {
    ...
    const learned = this.progress.getLearned(lesson.id, 'input');
